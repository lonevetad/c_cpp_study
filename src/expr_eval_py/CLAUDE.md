...
- Output: `_expr_eval_core.dylib` (Apple ld64; no `-static-libgcc`, no `-fuse-ld=lld`)
...
# macOS — from src/expr_eval_py/:
make all               # builds _expr_eval_core.dylib
make test              # build + run pytest
make clean             # remove .dylib
```

**What `all` does:**
1. Compiles `bridge/bridge.cpp` + 10 C++ sources incrementally into `build/` object files, links to `expr_eval_py/_expr_eval_core.dll` (Windows) / `_expr_eval_core.so` (Linux) / `_expr_eval_core.dylib` (macOS). `-MMD -MP` auto-generates `.d` dep files; only changed TUs recompile.
...
4. **macOS:** `.dylib` built with `-dynamiclib -Wl,-dead_strip`; no `-static-libgcc` (Apple clang has no libgcc); no extra runtime copy needed.
...
│   ├── _expr_eval_core.dylib    ← macOS:   generated by make (not in VCS)
