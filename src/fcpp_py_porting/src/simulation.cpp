...
namespace sim_tags {
    struct exported_state {};   // last exported state, readable via C ABI
    struct position {};         // Vec3 position snapshot (3 doubles)
}
...
namespace fcpp { namespace coordination {
...
namespace fcpp { namespace component {

DECLARE_COMBINE(headless_simulator,
    simulated_connector, simulated_map, simulated_positioner,
...
void fcpp_sim_run(double end_time) {
    if (g_sim) g_sim->run(static_cast<times_t>(end_time));
}
...
int32_t fcpp_sim_get_all_states(int32_t* ids_out, double* states_out, int32_t max_n) {
    if (!g_sim || !ids_out || !states_out) return 0;
