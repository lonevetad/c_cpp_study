...
ActivePingStrategy
    ...
    seconds.  Requires the compiled FCPP binary to implement the ping handler.
...
class ActivePingStrategy(LivenessStrategy):
    ...
    **Requires** the compiled FCPP binary to implement the ``ping`` command
    handler.  Without that runtime support the strategy always returns False.
