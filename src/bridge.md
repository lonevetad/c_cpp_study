...
**Status**: All 7 Phases + v1.4–v2.0 Implemented  
...
> **Note**: The layout below reflects the current (v2.0) one-file-per-class structure.
> The original planning layout used fewer, larger files; this evolved during implementation.

```
src/fcpp_bridge/
├── README.md                      Project overview, phase table, changelog
├── DSL_GUIDE.md                   Complete Python DSL reference
├── TUTORIAL_simple.md             Beginner walkthrough
├── TUTORIAL_in_depth.md           Production walkthrough
├── DEPLOYMENT.md                  Physical deployment guide
│
├── python_dsl/                    Phase 1: DSL layer
│   ├── primitives/                64 primitive classes (one .py each)
│   ├── types/                     CppType + 14 proxy classes + TemplateParam
│   ├── validators/                ValidationRule ABC, 5 rules, AggregateValidator
│   └── decorators/                6 _Mixin* classes + @aggregate_function
│
├── transpiler/                    Phase 2: Python → C++ code gen
│   ├── python_ast_visitor.py      AST → C++ statement visitor (guards, OR patterns, …)
│   ├── cpp_code_builder.py        Assemble complete C++ source
│   ├── transpiler_core.py         Transpiler public API
│   └── _constants.py              _FCPP_PRIMITIVES dict (64 entries, 7 headers)
│
├── compiler/                      Phase 3: Build pipeline
│   ├── program_cache.py           SHA-256 hash-based caching
│   ├── compiler_core.py           GCC subprocess wrapper
│   ├── cmake_generator.py         CMakeLists.txt generation
│   └── compilation_error_parser.py Structured GCC diagnostic parsing
│
├── runtime/                       Phase 4: Generated C++ runtime headers
│   └── runtime_generator.py       Emits ipc_server.hpp, state_serializer.hpp, …
│
├── ipc/                           Phase 4B / v1.0–v1.3: IPC + physical nodes
│   ├── _ipc_node_base.py          Shared listener pipeline + liveness
│   ├── swarm_process.py           Simulation subprocess lifecycle
│   ├── physical_node.py           Physical device deployment
│   ├── device_manager.py          Heterogeneous fleet management
│   ├── liveness_strategy.py       PassiveHeartbeat / ActivePing / AlwaysAlive ABCs
│   ├── listener_proxy.py          Sequential / parallel-async listener dispatch
│   ├── unix_socket_backend.py
│   ├── http_backend.py
│   └── grpc_backend.py
│
├── grammar/                       Phase 5: Language parser
│   ├── aggregate_language_parser.py  Recursive-descent parser
│   ├── antlr_parser.py            antlr4 wrapper with fallback
│   ├── ast_node.py
│   └── __antlr_gen/               Auto-generated by generate_antlr.py
│
...
│   └── metrics_summary.py
│
├── visualization/                 Phase 7: Terminal + matplotlib visualizers
│   ├── text_dashboard.py
│   └── swarm_visualizer.py
│
├── examples/                      Demo programs (7 main + utilities)
│   ├── abstract_example.py        AbstractExample base class (v2.0)
│   ├── _example_utils.py          Shared helpers: neighbors_of, build_positions, SPAWN_STATUS_*
...
│   └── logs/                      ← Per-node log files (git-ignored)
│
├── tests/                         624-test pytest suite
│   ├── dsl/                       Phase 1 (6 files)
│   ├── transpiler/                Phase 2 (3 files, includes guard/OR tests)
│   ├── compiler/                  Phase 3 (3 files)
│   ├── ipc/                       Phase 4 (7 files)
│   ├── grammar/                   Phase 5 (5 files)
│   ├── metrics/                   Phase 6 (6 files)
│   └── visualization/             Phase 7 (4 files)
│
├── development_history/           Design decisions + session records
│   ├── EXAMPLES_JOURNAL.md
│   ├── FUTURE_EXERCISES.md
│   ├── match_guard_or_pattern.md  Guard clauses + OR patterns (v2.0)
│   ├── V1_9_PLAN.md
│   └── …
│
...
17. [x] C++-alike DSL control flow + per-step pipeline CLI (v1.4) — +32 tests
    - `PythonAstVisitor` upgraded to full statement-level transpilation:
      `if`/`elif`/`else`, `while`, `for range(...)`, `match/case` → C++ `switch`,
      variable assignments (`auto` on first use, plain on re-use), `return`,
      `pass`, `break`, `continue`; ternary expressions, boolean operators
      (`and`/`or`/`not`), unary operators, list/tuple literals, method calls,
      built-ins (`abs`, `int`, `float`, `bool`)
    - `Transpiler._transpile_method_body` replaces `_transpile_method_return`
    - `AggregateProgram.g4` extended with `stmt`, `ifStmt`, `whileStmt`, `forStmt`, `switchStmt`, `assignStmt`, `returnStmt`
    - `DSL_GUIDE.md` — comprehensive reference for all primitives, types, mixins, C++-alike grammar, transpilation pipeline, limitations, and four full examples
    - `examples/end_to_end.py` — `--steps` / `--from` CLI flags for per-step execution

18. [x] worker_role_assignment.py example (v1.5 → v1.9) — +0 new tests (example-only)
    - v1.5: `WorkerRole` (8 values) + `match/case` → C++ `switch` + `spawn`-based message routing
    - v1.6: `self_uid()` primitive (`→ node.uid`, no CALL counter); `SelfUid` class; +1 test
    - v1.7: `RoleCommunicationType` enum; `REPEATER` rename; `RUBBLES_REMOVER` as endpoint
    - v1.8: `_example_utils.py` with `report_validation`/`report_transpilation` helpers; library `print()` → `get_logger()`
    - v1.8.1: `ROLE_CYCLE` with 5 extra REPEATER slots; `DEVICES=26`; `ROLE_COMM_TYPE` frozenset-unpack bug fix
    - v1.8.2: `WorkerRole.X.value` in `case` labels and comparisons; enum members in `ROLE_CYCLE`
    - v1.8.3: `ADDITIONAL_REPEATERS_EACH_CYCLE`, `FULL_ROLES_ASSIGNMENT_CYCLES_ROUNDS` named constants
    - v1.8.4: transpiler enum constant-folding (`compute.__globals__` → integer literals for `case` labels); +5 tests
    - v1.8.5: import path fix (`from fcpp_bridge.examples._example_utils import …`) in all 6 examples
    - v1.9: `communication_roles_assignment.py` new example; `neighbors_of` + `SPAWN_STATUS_*` added to `_example_utils.py`; all 6 existing examples refactored to use shared helpers

    **v1.9 examples inventory** (all in `examples/`):
    | File | Primitives used |
    |------|----------------|
    | `spreading_collection.py` | `rectangle_walk`, `abf_distance`, `mp_collection`, `broadcast` |
    | `channel_broadcast.py` | `rectangle_walk`, `bis_distance`, `broadcast` |
    | `collection_compare.py` | `rectangle_walk`, `abf_distance`, `sp_collection`, `mp_collection`, `wmp_collection` |
    | `message_dispatch.py` | `rectangle_walk`, `bis_distance`, `sp_collection`, `spawn`, `old` |
    | `chain_decaying.py` | `nbr`, `min_hood` |
    | `worker_role_assignment.py` | `bis_distance`, `nbr`, `min_hood`, `count_hood`, `sp_collection`, `spawn`, `old`, `self_uid()`, `match/case` |
    | `communication_roles_assignment.py` | `bis_distance`×2, `nbr`, `min_hood`, `old`×2, `broadcast`, `self_uid()`, `match/case` |

19. [x] match guard clauses + OR patterns (v2.0) — +8 transpiler tests
    - `PythonAstVisitor.visit_Match` extended:
      - Guard clauses: `case X if cond:` → `case X: if (cond) { ... } break;`
      - OR patterns: `case A | B:` → `case A: case B:` (C++14 fallthrough labels)
      - Guard + OR combined: `case A | B if cond:` wraps shared body in `if` block
    - `DSL_GUIDE.md §6.7` updated with guard and OR pattern docs, generated C++ examples, and expanded limitations
    - `development_history/match_guard_or_pattern.md` — full feature doc with AST notes, limitations, CALL-counter warning

20. [x] AbstractExample base class + examples refactoring (v2.0)
    - `examples/abstract_example.py`: `AbstractExample(ABC)` — Template Method pattern:
      - Abstract: `log_prefix`, `initial_positions()`, `initial_states(positions)`, `round_step(round_num, positions, states)`, `log_header(node_id, state)`, `log_line(round_num, node_id, state)`
      - Optional hooks: `on_simulation_start()`, `on_simulation_end()`, `on_round_complete(round_num, positions, states)`
      - `run(num_rounds)` — manages node lifecycle, per-node log files, dynamic add/remove
    - All 7 main examples refactored: `_demo_simulate()` converted to `class XxxExample(AbstractExample)`
    - Nodes represented as `dict[int, state]` throughout (not `range(N)`); nodes join/leave by presence in the dict
...
- `FUTURE_EXERCISES.md` — FE-1 through FE-8 (spawn, dynamic sinks, mobile nodes, area partition, receiver-UID fix, min-UID election, store-and-forward TTL, adaptive range)
